...
Convolution_xtensa::Convolution_xtensa()
{
    one_blob_only = true;
    support_inplace = false;
}

int Convolution_xtensa::load_param(const ParamDict& pd)
{
...
int Convolution_xtensa::load_model(const ModelBin& mb)
{
    if (dynamic_weight)
        return 0;
...
int Convolution_xtensa::create_pipeline(const Option& opt)
{
    if (dynamic_weight)
        return 0;
...
int Convolution_xtensa::forward(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
#if NCNN_INT8
...
int Convolution_xtensa::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
...
void Convolution_xtensa::make_padding(const Mat& bottom_blob, Mat& bottom_blob_bordered, const Option& opt) const
{
    make_padding(bottom_blob, bottom_blob_bordered, kernel_w, kernel_h, opt);
}

void Convolution_xtensa::make_padding(const Mat& bottom_blob, Mat& bottom_blob_bordered, int _kernel_w, int _kernel_h, const Option& opt) const
{
    int w = bottom_blob.w;
    int h = bottom_blob.h;
...
int Convolution_xtensa::forward_int8(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
    int w = bottom_blob.w;
    int h = bottom_blob.h;
    ...
    //     NCNN_LOGE("Convolution_xtensa input %d x %d  ksize=%d %d  stride=%d %d", w, h, kernel_w, kernel_h, stride_w, stride_h);
